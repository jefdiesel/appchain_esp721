...
function UploadContent() {
  ...
  // Screenshot all owned names (backfill) - single request, one browser session
  const screenshotAll = async () => {
    if (ownedNames.length === 0) return;
    setScreenshotting(true);
    setScreenshotResults(null);
    const allNames = ownedNames.map(n => n.name).join(',');
    const firstPunycode = toPunycode(ownedNames[0].name);
    try {
      const res = await fetch(
        `https://${firstPunycode}.chainhost.online/_screenshot-all?key=ch0st-cl34r-2024&names=${encodeURIComponent(allNames)}`
      );
      if (res.ok) {
        const data = await res.json();
